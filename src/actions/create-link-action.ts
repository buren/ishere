...
import { LinkKVSchema, CreateLinkRequestBody, Action } from '../types';
...
export const createLinkAction: Action<CreateLinkRequestBody, LinkWithUrls> = async ({ data, url, env }) => {
	...
	const link: LinkKVSchema = { destinationUrl, id, namespace, createdAt, updatedAt: createdAt };
	await env.KV.put(id, JSON.stringify(link), { expirationTtl });

	return {
		data: linkWithUrl(url, link),
		waitFor: [dbCreateLink(env.D1, { id, destinationUrl })],
	};
};
